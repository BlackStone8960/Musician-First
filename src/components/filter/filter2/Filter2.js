...
const Filter2 = (props) => {
  ...
  const [numOfGenres, setNumOfGenres] = useState(
    genreList[props.match.params.category].length
  );

  const makeIsClickedCheckList = () => {
    let checkListArr = [];
    for (let i = 0; i < numOfGenres; i++) {
      checkListArr.push(false);
    }
    setIsClicked(checkListArr);
  };

  useEffect(() => {
    makeIsClickedCheckList();
  }, []);
  return <h1>filter2</h1>;
};
...
//       this.setState((prevState) => {
//         return {
//           selectCount: this.state.clicked[index]
//             ? prevState.selectCount + 1
//             : prevState.selectCount - 1,
//           clicked: this.state.clicked,
//         };
//       });
